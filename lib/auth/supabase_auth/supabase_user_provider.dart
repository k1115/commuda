...
class CommudaSupabaseUser extends BaseAuthUser {
  CommudaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> commudaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CommudaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
